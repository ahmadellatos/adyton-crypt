...
class NotifBar(ctk.CTkFrame):
    """Colored notification strip always present in layout — no shifting."""
...
class AppBrankas(ctk.CTk):
    def __init__(self):
        ...
        self.tabview = ctk.CTkTabview(
            self, width=470, height=568, corner_radius=12,
            fg_color="#12141F",
    ...
    def setup_tab_kunci(self):
        ...
        self.lbl_path_folder.pack(anchor="w", padx=14, pady=(0, 10))
        ...
        # Menggunakan self.row_pw agar bisa diakses di _on_pw_kunci_change
        ...
        # Strength bar — Sekarang menggunakan self.row_str dan TIDAK di-pack di awal
    ...
    def _on_pw_kunci_change(self, _=None):
        ...
        if s < 0:
            # Jika kolom password kosong, sembunyikan baris indikator kekuatan
            self.row_str.pack_forget()
            self.strength_bar.set(0)
            self.lbl_strength.configure(text="")
        else:
            # Jika ada isinya, munculkan tepat di bawah row_pw
    ...
    def proses_kunci(self):
        ...
        threading.Thread(
            target=lambda: self.after(
                0, lambda: self._selesai_kunci(*kunci_brankas_logic(path_snap, pw_snap))
            ),
            daemon=True
        ).start()
    ...
    def _selesai_kunci(self, sukses: bool, pesan: str):
        ...
        if sukses:
            ...
            self.strength_bar.set(0)
            self.lbl_strength.configure(text="")
            self.row_str.pack_forget() # Sembunyikan kembali setelah sukses
